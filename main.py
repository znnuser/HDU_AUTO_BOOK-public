...
class SeatAutoBooker:
    ...
    def book_favorite_seat(self, user_config, seat_config):
        ...
        if seat_type == "自习室":
            start_time = datetime.now().replace(hour=7-time_zone, minute=0, second=0, microsecond=0)
            end_time = datetime.now().replace(hour=9-time_zone, minute=15, second=0, microsecond=0)
        else:
